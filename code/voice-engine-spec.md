...
> **9-2 精简提案生效时走 V1-Lite 路径**（见 audio-voice-strategy.md §10）：§2/§3 的打分仲裁引擎、话术表 OTA、48h 去重**整体后置到 V2**；V1 用三档固定优先级（高:出框/破纪录/越昨/达标/时间到 · 中:开场白/结算/报时/时间节点 · 低:整十报数）+ 每 intent 一个每轮布尔位实现，语音包 ~130 条纯离线，记忆快照缩到 4 字段（yesterday_count / best_session / best_at / streak）。9-2 补充：自由模式**每满 1 分钟报时**（数字拼接；第 20 分钟让位长轮关怀、第 29 分钟起让位上限预告）；**结算小结 ≤2 句、端上拼接**——§6-3 的结算句云 TTS 预取在 V1 一并不做（云端动态生成耗时，短小结用固定句+数字干声足够）；开场白**保留昵称点名**——称呼音频在设置/修改昵称时一次性云端生成缓存（`POST /voice/nick-clip`，改名重生成），未写/失败回落无称呼版，云端**每日**预生成管线仍然不做。下表 M-A/M-C 对应并简化为 M-A′/M-B′ 两步。
