...
| `history` | map<date, {count, best, n, ms, done}> | 每日聚合:累计次数 / **单轮最高 best(榜单口径)** / 轮数 n / 累计净时长 ms / 是否打卡 | 每次结算累加(**单轮从 0 计,当日跨轮累加**;best 取 max);按 session 存明细、按天聚合展示;打卡条件 = 当日 ≥ 目标 或 ≥50 |
