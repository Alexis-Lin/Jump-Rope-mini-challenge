# Body Park · 跳绳小挑战（Jump Rope Mini Challenge）

**打开就能跳的摄像头跳绳小应用** —— Atom 智能设备(466×466 圆屏 @10–15fps + 摄像头 + 后端 AI 姿态/计数管线)。
核心诉求:简单易上手,不用动脑,打开就能用。

当前版本:**proto-v0.29**(2026-08-18,已并入 8-18 评审会决议)· Atom 端(手机端暂缓)。

---

## 仓库结构

```
docs-and-demo/                     文档与 Demo(一站式)
├── demo.html                      ★ 唯一入口(Demo + 手册合一):顶部页签切换
│                                    Tab1 交互演示(左侧控制面板 +「初始状态」开关)
│                                    · Tab2 火柴人渲染说明(实时交互)· Tab3 起 关键逻辑
│                                    / 接口字段 / 视觉标准;加 ?app=1 = 纯 Demo(设备 WebView 直载)
├── jump-rope-mini-prd.md          PRD 完整版(含产品设计说明附录) —— 开发以此为准
├── audio-voice-strategy.md        课中音频整体策略(音效/BGM/TTS 人声,三层架构+资产清单)
├── meetings/                      评审会纪要存档(08-18 跳绳产品功能方案讨论)
├── figure-rendering.md            AI 火柴人渲染说明(33 关键点接口 + 几何 + 动效管线)
└── ui/                            全部 UI 图(en/ zh/ 各 22 屏 + 火柴人规格卡 PNG)
code/                              工程实现代码 + 后端
├── backend-spec.md                后端字段/排行榜/实现伪代码 + ATOM 用户档案模型字段对齐
├── backend/                       后端参考实现(FastAPI+Redis 单文件可跑:月度三榜/脱敏/校验/日卡·结果卡/补记)
└── (include/ src/ Makefile)       嵌入式参考:纯 C(已对齐 2026-08-18 决议版) + 接口契约 + WebView 方案
backup/                            过程工具(截图生成器 / 规格卡源文件)
```

## 快速体验

```
双击 docs-and-demo/demo.html          # ★ 一站式入口:交互演示 + 渲染说明 + 关键逻辑(顶部页签)
docs-and-demo/demo.html?lang=zh       # 中文版(默认英文)
docs-and-demo/demo.html?app=1         # 纯 Demo 模式(隐藏手册外壳;设备 WebView 用这个)
```

Demo 控制条:自动演示 / 语音 / 10·13·15fps / 1×2×3×;空格键 = 模拟跳一下。

## 产品速览

- **流程**:入口卡(首页阶段不播语音——8-18)→ 首页卡组(首页/打卡记录/排行榜/最佳战绩速览;追加记录从打卡记录页「补记」进入)→ 模式选择(不限时自由跳 · 限时 1'/2' 默认 1';仅并脚跳提示)→ 3·2·1·**BodyPark** 即时开跳 → 训练(自由跳 30 分钟上限)→ 结果页(点「完成」后弹新纪录恭喜帧)
- **训练屏**:进度环(限时=60 格刻度环)+ 打击计数(逢十重击+全身金光)+ 骨骼火柴人 + 随机彩绳(8 色池,每 50 换色)+ 金色涟漪 + 每 50 得星/燃起来
- **激励**:连胜打卡 + 三榜排行(自由单轮/1 分钟/连续打卡,**按月度刷新**,实时超越)+ 星星/隐藏数字/场景解锁;常规节点全语音化;每次训练自动生成 466×466 结果分享卡(同步 App 云相册)
- **硬约束**:字号 6 档(18/22/24/28/32/36)、黑白灰绿品牌色、单色 SVG 图标、10–15fps 离散帧、中英双语

## 团队接力

- **设计师**:`docs-and-demo/ui/` + Demo;待产出:ATOM 吉祥物正式形象(命名定稿,原工作名 Bono)、燃起来细化、分享海报、轻量音效包(不复用现有课程音效)(硬约束 PRD §7)
- **产品**:PRD §12 开放问题;星星数值复核;体测国标分表
- **AI 团队**:figure-rendering.md + 手册"火柴人渲染"页(可交互)
- **端上工程**:`code/`(纯 C 参考已对齐决议版,或 WebView 直载 demo.html?app=1)
- **服务端**:code/backend-spec.md(规格)+ code/backend/(可运行参考:月度三榜 ZADD GT/显示名脱敏/校验/日卡·结果卡云相册/追加记录;账号级字段以 ATOM-UserGoalPreference-and-OnBoarding 仓库为源)
