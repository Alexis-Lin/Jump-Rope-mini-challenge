...
`feedPose(pts)`(C:`atom_app_feed_pose`):**BP-28 拓扑,28 个关键点**(对应后台 `KeypointNameEnum`),归一化 0–1、原点左上,建议 ≥15Hz;500ms 无数据自动回落内置模拟动作。每点 `{x, y, v?}`(v = 置信度,可选)。
...
**待 AI 团队确认的 4 件事**:① 镜像方向(用户举右手,画面上是左还是右);② `v` 置信度字段有无与低置信约定;③ 实际推流帧率;④ head(9) 语义是头心还是头顶(当前按头心渲染,若为头顶则圆心下移 r/2)。
